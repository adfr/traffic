...
def load_data(data_dir):
    ...
    for i in range(NUM_CATEGORIES):
        dir_i = os.path.join(data_dir,str(i))
        list_files = os.listdir(dir_i)
        print("extracting directory ",str(i))
        for j in list_files:
            images.append( cv2.resize( cv2.imread(os.path.join(dir_i,j)), (IMG_WIDTH, IMG_HEIGHT) ,interpolation = cv2.INTER_AREA  ) )
            labels.append(i)
